package com.bharath.order_service.config;

import com.bharath.order_service.ApplicationProperties;
...
class RabbitMQConfig {
    ...
    // configuring direct exchange for rabbitmq
    @Bean
    DirectExchange exchange() {
        return new DirectExchange(properties.orderEventsExchange());
    }

    //creating queues for new, delivered, cancelled and error orders and binding them to the exchange
    @Bean
    ...
    //configuring rabbit template to use jackson2jsonmessageconverter to convert messages to json format before sending
    @Bean
